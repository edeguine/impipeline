...
def make_progress_callback(shape, output_directory, save_every=2, output_name='', final_iter=-1):
...
def synthesize_animation(pyramid_model, gram_model, target_grams,
        ...
        x0=None, max_iter=200, save_every=2, tol=1e-9, output_name=''):
    ...
    print("About to start minimizing...", flush=True)
    result = minimize(optimize_me, np.ravel(x0), jac=True, method="l-bfgs-b",
                      callback=make_progress_callback(generated_shape, output_directory, save_every=save_every, output_name = output_name, final_iter = max_iter),
                      tol=tol,
                      #bounds=bounds,
                      options={'disp': True, 'maxiter': max_iter})
    return result
